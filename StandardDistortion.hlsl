...
struct FDistortionPassConstants {
...
ConstantBuffer<FDistortionPassConstants> DistortionPass;
Texture2D  DistortionPass_SceneTextures_SceneDepthTexture;
SamplerState  DistortionPass_SceneTextures_PointClampSampler;
...
{
	return ConvertFromDeviceZ(Texture2DSampleLevel(DistortionPass_SceneTextures_SceneDepthTexture, DistortionPass_SceneTextures_PointClampSampler, ScreenUV, 0).r);
}
float CalcSceneDepth(uint2 PixelPos)
{
	float DeviceZ = DistortionPass_SceneTextures_SceneDepthTexture.Load(int3(PixelPos, 0)).r;
...
void CalcPixelMaterialInputs(in out FMaterialPixelParameters Parameters, in out FPixelMaterialInputs PixelMaterialInputs)
{
	...
	float3 Local4 = lerp(((float3)1.00000000),Material.PreshaderBuffer[0].yzw,Material.PreshaderBuffer[0].x);
	float Local5 = dot(Parameters.WorldNormal,Parameters.CameraVector);
	float Local6 = max(0.00000000,Local5);
	float Local7 = (1.00000000 - Local6);
	float Local8 = abs(Local7);
	float Local9 = max(Local8,0.00010000);
	float Local10 = PositiveClampedPow(Local9,5.00000000);
	float Local11 = (Local10 * (1.00000000 - 0.04000000));
	float Local12 = (Local11 + 0.04000000);
	...
	PixelMaterialInputs.Refraction = float3(float2(Local12,0.0f),Material.PreshaderBuffer[1].x);
	...
	)
{
	...
	float2 BufferUVDistortion = ComputeBufferUVDistortion(
		...
		DistortionPass.DistortionParams, ScreenUV, RefractionData, true, EyeIndex);
